import 'package:flutter/material.dart';

// color list
Color color1 = Color(0xff63af5b) ; // easy 진행바, easy 버튼 눌렀을 때 반응색, 정답 화면에서 다음 문제 버튼 눌렀을 때 반응색
Color color2 = Color(0xffe36d3f) ; // hard 진행바, hard 버튼 눌렀을 때 반응색, 정답 화면에서 다음 문제 버튼 눌렀을 때 반응색
Color color3 = Colors.black54 ; // 정답 버튼 안에 있는 숫자 글자 색상

Color color4 = Color(0xff4b7947) ; // 정답 창 안에 정답일 때 글자 색
Color color5 = Color(0xffacd0a8) ; // 정답 창 안에 정답일 때 배경 색
Color color6 = Color(0xff79474e) ; // 정답 창 안에 오답일 때 글자 색
Color color7 = Color(0xffacd0a8) ; // 정답 창 안에 오답일 때 배경 색

Color color8 = Color(0xffdedede) ; // firstProblemTypeList 의 타일의 테두리 색

