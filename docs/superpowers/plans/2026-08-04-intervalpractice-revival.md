...
final class ProblemGenerator {
  ...
  /// > **실행 중 판명:** 이 사전 필터만으로는 부족하다. 홑임시표 분기도
  /// > 답할 수 없는 음정을 만들 수 있다 — 예: `E5♯ + F5♭` → 하행 `dd2`.
  /// > 따라서 `next()`의 재시도 루프에서 **생성된 후보 전체**를
  /// > `_isAnswerable`로 검증하는 것이 실제 보증이고, 아래 메서드는
  /// > 값싼 사전 필터일 뿐이다.
  ///
...
> **실행 중 정정:** 원래 여기에 "음표 머리가 어색하면 이미 존재하는 흰색 버전
> `whole_note_lean_all_white.png`를 쓰라"고 적었는데, **그 에셋은 불투명 픽셀이
> 0개인 완전히 빈 이미지였다.** 썼다면 다크모드에서 음표머리가 사라진다.
> 구현자가 픽셀 데이터를 직접 검사해서 발견했고, 해당 파일은 삭제했다.
>
> 실제 확인 결과 `whole_note_lean.png`의 빈 타원 부분은 흰색이 아니라 **알파 0인
> 투명 영역**이라, RGB 반전이 알파를 보존하므로 검은 테두리만 흰 테두리가 되고
> 안쪽은 투명하게 남는다 — 별도 대안 에셋이 필요 없다.
...
## 부록: 실기기/시뮬레이터 검증에서 추가로 발견한 결함 (실행 중 기록)

계획서는 "시뮬레이터가 없어 컴파일과 테스트만 검증했다"고 적었으나, 실제로는 iOS 시뮬레이터가 사용 가능했다(`flutter devices`만 보고 `xcrun simctl list`를 확인하지 않은 탓). 앱을 띄우고 나서 **정적 검증으로는 잡히지 않는 결함 3건**이 나왔다.

| 결함 | 발견 방법 | 영향 |
|---|---|---|
| 홈 타일 아이콘이 다크모드에서 안 보임 | 스크린샷 픽셀 대비 측정 (1.74:1) | Task 20이 `staff_view.dart`만 반전 처리하고 홈 화면을 빠뜨림 |
| 탭 지시선이 다크모드에서 검정 | 사용자 육안 제보 | `BorderSide(width: 2)`의 색 기본값이 검정 → 구분선이 끊긴 것처럼 보임 |
| 정답 버튼 가로 넘침 | 6개 모드 골든 렌더 중 예외 | `RenderFlex overflowed by 14 pixels` — **375pt 기기(iPhone SE, 13 mini)에서 실제 발생** |

**교훈:** 위젯 테스트는 "위젯이 존재한다"를 증명하지만 "보인다/들어맞는다"는 증명하지 못한다. 다음 두 가지를 안전망으로 남겼다.

- `test/ui/quiz_golden_test.dart` — 오선지를 라이트/다크로 렌더해 PNG 생성
- `test/ui/quiz_modes_golden_test.dart` — 6개 모드 × 2 테마 = 12조합 렌더, **`takeException()`이 null인지 단언**하므로 넘침류 레이아웃 예외가 재발하면 즉시 실패한다

골든은 `@Tags(['golden'])`로 표시하고 CI에서 `--exclude-tags golden`으로 제외한다(폰트 래스터화가 macOS와 Ubuntu에서 달라 이미지 비교가 재현되지 않음). 갱신은 `flutter test --update-goldens`.

### 남은 관찰 사항 (수정하지 않음)

임시표가 음표 머리에 겹쳐 그려진다. 기존 앱의 좌표(`dTop`/`dLeft`)를 그대로 옮긴 결과이며 원본과 동일한 모습이다. 악보 관례상 임시표는 음표 **왼쪽**에 놓이는 게 맞지만, "레이아웃은 유지하고 시각 레이어만 현대화"라는 범위 결정에 따라 임의로 바꾸지 않았다. 바꾸려면 별도 결정이 필요하다.
